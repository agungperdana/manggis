...
export default function PartyRelationshipAddForm({token, partyCode}) {

  const navigation = useHistory();
  const [form] = Form.useForm();
  const [start, setStart] = React.useState(moment());
  const [end, setEnd] = React.useState(null);
  const [toParty, setToParty] = React.useState(null);
  ...
  const create = async () => {
    ...
    try {
      ...
      if(start && type && toParty && token && partyCode) {
        let response = await fetch('https://127.0.0.1:8585/partys/relationships/create', {
          method: 'POST',
          headers: {
            Accept: 'application/json', 
            'Content-Type': 'application/json',
                ...
                <Form style={{width:"60%", alignSelf:"flex-start", padding:3}} 
                  ...
                  <Form.Item label="Sart Date" name="start" rules={[{ required: true }]}>
                    ...
                    <Form.Item label="To Party" name="toParty">
                        <Select name="toParty" onChange={(txt)=>setToParty(txt)}>
                        {
                          partys.map((party)=>{
