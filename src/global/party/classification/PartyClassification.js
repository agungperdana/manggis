...
export default function PartyClassification({token, partyCode}) {
    ...
    const [classificationForm, setClassificationForm] = React.useState({});
    ...
    const column = [
        ...
        {title:"", dataIndex:"", key:"Action", width:100, render:(txt, row)=>(
            <RowToolbar delAction={()=>{remove(row.id)}} 
                        ...
                        printAction={()=>{console.log();}}/>
       )},
      ]

    return (
        <>
            <TableTopBar addDocAction={()=>setOpenAddForm(true)}
            ...
            <PartyClassificationAddForm token={token} partyCode={partyCode} visible={openAddForm} closeAction={()=>{
                loadData();
                setOpenAddForm(false);
            }}/>
            <PartyClassificationEditForm 
                        token={token} 
