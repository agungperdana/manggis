...
export default function PartyRole({token, partyCode}) {
    ...
    const loadData = async () => {
        ...
        try {
          ...
          if(json.status) {
              setData(json.result?.partyRoles);
          }
        } 
        catch (error) {
    ...
    const remove = async (id) => {
        ...
        try {
            ...
            if(id) {
                ...
                let response = await fetch('https://127.0.0.1:8585/partys/roles/delete', {
                    method: 'DELETE',
                    headers: {
                        Accept: 'application/json', 
                        'Content-Type': 'application/json',
    ...
    const column = [
        ...
        {title:"", dataIndex:"", key:"Action", width:100, render:(txt, row, index)=>(
            <RowToolbar delAction={()=>{remove(row.id)}} 
                        editAction={()=> {
                            navigation.push("/global/party/edit/role/edit/",{data:row});
                        }}
                        printAction={()=>{
                            setRow(row);
                            setVisible(true);
    ...
    return (
        <>
            <TableTopBar addDocAction={()=>{
                            navigation.push("/global/party/edit/role/add/");
                         }}
            ...
            <PartyRolePrint visible={visible} data={row} cancelAction={()=>setVisible(false)} confirmAction={()=>setVisible(false)}/>
        </>
    );
}